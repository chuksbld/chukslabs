...
SELECT account_id, sum(amount) as total_sum
FROM bank.loan
GROUP BY account_id
...
SELECT date, count(loan_id) AS loan_issued
FROM bank.loan
WHERE date < 930907
GROUP BY date
ORDER BY date  DESC;
...
SELECT date, duration, count(loan_id) as loan_id
FROM bank.loan
WHERE date BETWEEN 971206 AND 971231
...
FROM bank.trans
WHERE type IN ('VYDAJ','PRIJEM') AND account_id = 396
GROUP BY type
ORDER BY type;
...
SELECT account_id,
		 CASE type
						WHEN 'VYDAJ' THEN 'INCOMING'
                        WHEN 'PRIJEM'THEN 'OUTGOING'
			  END AS 'English',
        round(sum(amount),-1) AS transaction_type
FROM bank.trans
...
select account_id, GROUP_CONCAT(floor(sum(amount)) ORDER BY type)
FROM bank.trans
WHERE account_id = 396
GROUP BY type
ORDER BY type